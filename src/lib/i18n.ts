export const dictionaries = {
  en: {
    meta: {
      title: "We're Getting Married!",
      description: "Join us for our special day",
    },
    login: {
      title: "You're Invited",
      subtitle: "Enter the password from your invitation to view the details",
      placeholder: "Enter password",
      button: "Enter",
      error: "Incorrect password. Please try again.",
    },
    nav: {
      home: "Home",
      schedule: "Schedule",
      rsvp: "RSVP",
      touristicInfo: "Travel Guide",
      weddingInfo: "Wedding Info",
      admin: "Admin",
      logout: "Logout",
    },
    home: {
      weAreGettingMarried: "We're Getting Married!",
      saveTheDate: "Save the Date",
      days: "Days",
      hours: "Hours",
      minutes: "Minutes",
      seconds: "Seconds",
      venueLabel: "Venue",
      dateLabel: "Date",
      joinUs: "We can't wait to celebrate with you!",
    },
    schedule: {
      title: "Schedule",
      dayLabel: "Day",
      subtitle: "Here's what to expect on our special week.",
      days: [
        {
          title: "Day 1: Thursday – The Secret Gathering",
          panelText:
            "The brave heroes ignite their legendary journey! Reuniting with closest kin in the hidden hearth of Mirto's domain for a feast of heartfelt tales and unbreakable bonds. (Shh... this sacred panel reveals itself only to the chosen few!)",
          locationName: "Mirto's House",
          locationUrl: "https://maps.app.goo.gl/kn94JwutcQFEfdpg9",
        },
        {
          title: "Day 2: Friday – The Grand Reunion",
          panelText:
            "All allies assemble! Heroes unite with friends and family at the wave-torn shore tavern from noon till stars claim the sky. Devour epic feasts, share endless laughs, and let the waves hype us for tomorrow's union.",
          locationName: "Alykes Beach",
          locationUrl: "https://maps.app.goo.gl/k1vWCCXF9Bb79aAG7",
        },
        {
          title: "Day 3: Saturday – The Ultimate Quest Begins",
          panelText:
            "Dawn of eternity! The heroes charge into the \"Journey Beyond\" atop Evia's majestic Dirfys mountains, within a mystical woodland realm. Ceremony and revelry await—don your finest armor and come prepared for feats of glory!",
          locationName: "Enchanted Woods",
          locationUrl: "https://maps.app.goo.gl/ExUjAww6jhaXCp8w5",
        },
        {
          title: "Day 4: Sunday – Dawn of the Survivors",
          panelText:
            "Victors emerge from the fray! Those who conquered the enchanted trials gather for a chill morning brew at a beloved local haven. Reflect, recharge, and toast to legends forged!",
          locationName: "Ktima Anthidon",
          locationUrl: "https://maps.app.goo.gl/qGqXru5bec2JYz356",
        },
        {
          title: "Day 5: Monday – The Wild Odyssey",
          panelText:
            "Final call for adventure! Brave heroes traverse Evia's rugged spine to the Aegean sea—a daring pilgrimage to a magic beach. Who is in for a wild ride with the crew?",
          locationName: "Chiliadou Beach",
          locationUrl: "https://maps.app.goo.gl/9vipXFbDTbtUQZGNA",
        },
      ],
    },
    rsvp: {
      title: "RSVP",
      subtitle: "Please let us know if you can make it",
      nameLabel: "Full Name",
      namePlaceholder: "Your full name",
      emailLabel: "Email Address",
      emailPlaceholder: "your@email.com",
      attendanceLabel: "Will you attend?",
      attendanceYes: "Joyfully accepts!",
      attendanceNo: "Regretfully declines",
      guestsLabel: "Number of Adults",
      kidsLabel: "Number of Children",
      phoneLabel: "Phone Number",
      phonePlaceholder: "+55 11 91234-5678",
      messageLabel: "Anything you would like to add/say?",
      messagePlaceholder: "Add any additional notes...",
      dietaryLabel: "Any dietary restrictions?",
      dietaryPlaceholder: "Please list allergies or dietary needs",
      submit: "Send RSVP",
      sending: "Sending...",
      successTitle: "Thank you!",
      successMessage: "Your RSVP has been received. We look forward to celebrating with you!",
      errorMessage: "Something went wrong. Please try again.",
      deadline: "Please RSVP by",
      sendAnother: "Send Another Response",
      alreadySubmittedTitle: "RSVP Already Sent",
      alreadySubmittedMessage: "You have already submitted your RSVP. If you need to make changes, please contact us directly.",
      rentCarLabel: "Will you rent a car?",
      rentCarYes: "Yes",
      rentCarNo: "No",
      eventDatesLabel: "Please confirm which dates you'll join the heroes in our",
      scheduleLinkText: "planned events",
      eventDatesNote: "(We will provide accommodation for the 5th and 6th of June)",
      eventOptions: {
        "2026-06-04": "<strong>4 June</strong>&nbsp;—&nbsp;Close-family dinner at <a href=\"https://maps.app.goo.gl/kn94JwutcQFEfdpg9\">📍 Mirto's House</a>",
        "2026-06-05": "<strong>5 June</strong>&nbsp;—&nbsp;All day at Alykes beach at <a href=\"https://maps.app.goo.gl/k1vWCCXF9Bb79aAG7\">📍 Alykes Beach</a>",
        "2026-06-06": "<strong>6 June</strong>&nbsp;—&nbsp;Wedding ceremony at <a href=\"https://maps.app.goo.gl/ExUjAww6jhaXCp8w5\">📍 Enchanted Woods</a>",
        "2026-06-07": "<strong>7 June</strong>&nbsp;—&nbsp;Brunch at Ktima Anthidon at <a href=\"https://maps.app.goo.gl/qGqXru5bec2JYz356\">📍 Ktima Anthidon</a>",
        "2026-06-08": "<strong>8 June</strong>&nbsp;—&nbsp;All day at Chiliadou beach at <a href=\"https://maps.app.goo.gl/9vipXFbDTbtUQZGNA\">📍 Chiliadou Beach</a>",
      },
    },
    touristicInfo: {
      title: "Travel Guide",
      subtitle: "Everything you need to know for your trip",
      introText:
        "Here we have listed some of our favourite attractions of the area, for those who are looking to explore the region further. <em>Note: If you want to visit Skiathos or Skopelos, please feel free to reach out for more info and guidance. We highly recommend to vacation there.</em>",
      mapsLinkText: "Google Maps list",
    },
    weddingInfo: {
      title: "Wedding Info",
      subtitle: "Important information for our guests",
      dressCodeTitle: "Dress Code",
      contactsTitle: "Contacts",
      giftsTitle: "Gifts",
      accommodationTitle: "Accommodation",
      contactNote: "Contact us for any matters",
    },
    admin: {
      title: "Admin Panel",
      subtitle: "Manage guests and view RSVPs",
      guestsTab: "Guests",
      rsvpsTab: "RSVPs",
      totalGuests: "Total Adults",
      totalRsvps: "Total RSVPs",
      attending: "Attending",
      declined: "Declined",
      addGuest: "Add Guest",
      editGuest: "Edit Guest",
      deleteGuest: "Delete",
      passwordLabel: "Password",
      passwordPlaceholder: "e.g. john2026",
      passwordRequired: "Password is required",
      nameLabel: "Guest Name",
      namePlaceholder: "Guest name",
      groupLabel: "Group",
      groupFriends: "Friends",
      groupFamily: "Family",
      groupAdmin: "Admin",
      save: "Save",
      noGuests: "No guests yet",
      noRsvps: "No RSVPs yet",
      rsvpEmail: "Email",
      rsvpPhone: "Phone",
      rsvpAttendance: "Attendance",
      rsvpYes: "Attending",
      rsvpNo: "Declined",
      rsvpGuests: "Adults",
      rsvpKids: "Kids",
      rsvpRentCar: "Renting a car",
      rsvpDietary: "Dietary restrictions",
      rsvpMessage: "Message",
      rsvpPassword: "Password used",
    },
    footer: {
      madeWithLove: "Made with love for",
    },
  },
  pt: {
    meta: {
      title: "Vamos Casar!",
      description: "Junte-se a nós no nosso dia especial",
    },
    login: {
      title: "Você está Convidado",
      subtitle: "Digite a senha do seu convite para ver os detalhes",
      placeholder: "Digite a senha",
      button: "Entrar",
      error: "Senha incorreta. Tente novamente.",
    },
    nav: {
      home: "Início",
      schedule: "Programação",
      rsvp: "Confirmação",
      touristicInfo: "Guia de Viagem",
      weddingInfo: "Informações",
      admin: "Admin",
      logout: "Sair",
    },
    home: {
      weAreGettingMarried: "Vamos Casar!",
      saveTheDate: "Reserve a Data",
      days: "Dias",
      hours: "Horas",
      minutes: "Minutos",
      seconds: "Segundos",
      venueLabel: "Local",
      dateLabel: "Data",
      joinUs: "Mal podemos esperar para celebrar com você!",
    },
    schedule: {
      title: "Programação do Dia",
      dayLabel: "Dia",
      subtitle: "Veja o que esperar no nosso dia especial",
      days: [
        {
          title: "Dia 1: Quinta-feira – A Reunião Secreta",
          panelText:
            "Os bravos heróis acendem sua jornada lendária! Reunindo-se com os parentes mais próximos no lar escondido do domínio de Mirto para um banquete de histórias sinceras e laços inquebráveis. (Shh... este painel sagrado revela-se apenas aos poucos escolhidos!)",
          locationName: "Casa da Mirto",
          locationUrl: "https://maps.app.goo.gl/kn94JwutcQFEfdpg9",
        },
        {
          title: "Dia 2: Sexta-feira – A Grande Reunião",
          panelText:
            "Todos os aliados, reúnam-se! Heróis unem-se com amigos e família na taverna da costa, golpeada pelas ondas, do meio-dia até que as estrelas dominem o céu. Devorem banquetes épicos, compartilhem risadas sem fim e deixem as ondas nos empolgar para a união de amanhã.",
          locationName: "Praia Alykes",
          locationUrl: "https://maps.app.goo.gl/k1vWCCXF9Bb79aAG7",
        },
        {
          title: "Dia 3: Sábado – A Jornada Suprema Começa",
          panelText:
            "Aurora da eternidade! Os heróis avançam na 'Jornada Além' no topo das majestosas montanhas Dirfys de Evia, dentro de um reino místico de floresta. Cerimônia e festejo aguardam—vista sua melhor armadura e venha preparado para feitos de glória!",
          locationName: "Enchanted Woods",
          locationUrl: "https://maps.app.goo.gl/ExUjAww6jhaXCp8w5",
        },
        {
          title: "Dia 4: Domingo – Aurora dos Sobreviventes",
          panelText:
            "Vencedores emergem da batalha! Aqueles que conquistaram as provações encantadas reúnem-se para um café matinal relaxado em um refúgio local querido. Reflita, recarregue e brinde às lendas forjadas!",
          locationName: "Ktima Anthidon",
          locationUrl: "https://maps.app.goo.gl/qGqXru5bec2JYz356",
        },
        {
          title: "Dia 5: Segunda-feira – A Odisseia Selvagem",
          panelText:
            "Última chamada para a aventura! Heróis corajosos atravessam a espinha acidentada de Evia até o mar Egeu—uma peregrinação ousada a uma praia mágica. Quem topa uma jornada selvagem com a tripulação?",
          locationName: "Praia Chiliadou",
          locationUrl: "https://maps.app.goo.gl/9vipXFbDTbtUQZGNA",
        },
      ],
    },
    rsvp: {
      title: "Confirmação de Presença",
      subtitle: "Por favor, confirme se poderá comparecer",
      nameLabel: "Nome Completo",
      namePlaceholder: "Seu nome completo",
      emailLabel: "Endereço de Email",
      emailPlaceholder: "seu@email.com",
      attendanceLabel: "Você virá?",
      attendanceYes: "Aceito com alegria!",
      attendanceNo: "Infelizmente não poderei",
      guestsLabel: "Número de Adultos",
      kidsLabel: "Número de Crianças",
      phoneLabel: "Telefone",
      phonePlaceholder: "+55 11 91234-5678",
      messageLabel: "Algo que queira acrescentar/dizer?",
      messagePlaceholder: "Adicione quaisquer observações...",
      dietaryLabel: "Alguma restrição alimentar?",
      dietaryPlaceholder: "Por favor, liste alergias ou necessidades alimentares",
      submit: "Enviar Confirmação",
      sending: "Enviando...",
      successTitle: "Obrigado!",
      successMessage: "Sua confirmação foi recebida. Esperamos celebrar com você!",
      errorMessage: "Algo deu errado. Tente novamente.",
      deadline: "Confirme até",
      sendAnother: "Enviar Outra Resposta",
      alreadySubmittedTitle: "Confirmação Já Enviada",
      alreadySubmittedMessage: "Você já enviou sua confirmação de presença. Se precisar fazer alterações, entre em contato conosco diretamente.",
      rentCarLabel: "Vai alugar um carro?",
      rentCarYes: "Sim",
      rentCarNo: "Não",
      eventDatesLabel: "Por favor confirme em quais datas você participará dos nossos",
      scheduleLinkText: "eventos planejados",
      eventDatesNote: "(Forneceremos acomodação para os dias 5 e 6 de junho)",
      eventOptions: {
        "2026-06-04": "<strong>4 de junho</strong>&nbsp;—&nbsp;Jantar íntimo em família na <a href=\"https://maps.app.goo.gl/kn94JwutcQFEfdpg9\">📍 Casa da Mirto</a>",
        "2026-06-05": "<strong>5 de junho</strong>&nbsp;—&nbsp;Dia todo na praia Alykes em <a href=\"https://maps.app.goo.gl/k1vWCCXF9Bb79aAG7\">📍 Praia Alykes</a>",
        "2026-06-06": "<strong>6 de junho</strong>&nbsp;—&nbsp;Cerimônia de casamento em <a href=\"https://maps.app.goo.gl/ExUjAww6jhaXCp8w5\">📍 Enchanted Woods</a>",
        "2026-06-07": "<strong>7 de junho</strong>&nbsp;—&nbsp;Brunch no <a href=\"https://maps.app.goo.gl/qGqXru5bec2JYz356\">📍 Ktima Anthidon</a>",
        "2026-06-08": "<strong>8 de junho</strong>&nbsp;—&nbsp;Dia todo na praia Chiliadou em <a href=\"https://maps.app.goo.gl/9vipXFbDTbtUQZGNA\">📍 Praia Chiliadou</a>",
      },
    },
    touristicInfo: {
      title: "Guia de Viagem",
      subtitle: "Tudo que você precisa saber para sua viagem",
      introText:
        "Aqui listamos algumas das nossas atrações favoritas da região, para quem quer explorar mais a área. <em>Nota: Se quiser visitar Skiathos ou Skopelos, fique à vontade para nos contactar para mais informações e orientações. Recomendamos fortemente umas férias lá.</em>",
      mapsLinkText: "Lista do Google Maps",
    },
    weddingInfo: {
      title: "Detalhes do Casamento",
      subtitle: "Informações importantes para nossos convidados",
      dressCodeTitle: "Traje",
      contactsTitle: "Contatos",
      giftsTitle: "Presentes",
      accommodationTitle: "Hospedagem",
      contactNote: "Contate-nos para qualquer assunto",
    },
    admin: {
      title: "Painel Admin",
      subtitle: "Gerencie convidados e veja confirmações",
      guestsTab: "Convidados",
      rsvpsTab: "Confirmações",
      totalGuests: "Total de Adultos",
      totalRsvps: "Total de RSVPs",
      attending: "Confirmados",
      declined: "Recusados",
      addGuest: "Adicionar Convidado",
      editGuest: "Editar Convidado",
      deleteGuest: "Excluir",
      passwordLabel: "Senha",
      passwordPlaceholder: "ex: joao2026",
      passwordRequired: "Senha é obrigatória",
      nameLabel: "Nome do Convidado",
      namePlaceholder: "Nome do convidado",
      groupLabel: "Grupo",
      groupFriends: "Amigos",
      groupFamily: "Família",
      groupAdmin: "Admin",
      save: "Salvar",
      noGuests: "Nenhum convidado ainda",
      noRsvps: "Nenhuma confirmação ainda",
      rsvpEmail: "Email",
      rsvpPhone: "Telefone",
      rsvpAttendance: "Presença",
      rsvpYes: "Confirmado",
      rsvpNo: "Recusado",
      rsvpGuests: "Adultos",
      rsvpKids: "Crianças",
      rsvpRentCar: "Alugando carro",
      rsvpDietary: "Restrições Alimentares",
      rsvpMessage: "Mensagem",
      rsvpPassword: "Senha utilizada",
    },
    footer: {
      madeWithLove: "Feito com amor para",
    },
  },
  el: {
    meta: {
      title: "Παντρευόμαστε!",
      description: "Ελάτε να γιορτάσουμε μαζί την ξεχωριστή μας μέρα",
    },
    login: {
      title: "Είστε Προσκεκλημένοι",
      subtitle: "Εισάγετε τον κωδικό από την πρόσκλησή σας για να δείτε τις λεπτομέρειες",
      placeholder: "Εισάγετε κωδικό",
      button: "Είσοδος",
      error: "Λάθος κωδικός. Παρακαλώ δοκιμάστε ξανά.",
    },
    nav: {
      home: "Αρχική",
      schedule: "Πρόγραμμα",
      rsvp: "Επιβεβαίωση",
      touristicInfo: "Ταξιδιωτικός Οδηγός",
      weddingInfo: "Πληροφορίες",
      admin: "Διαχείριση",
      logout: "Αποσύνδεση",
    },
    home: {
      weAreGettingMarried: "Παντρευόμαστε!",
      saveTheDate: "Κρατήστε την Ημερομηνία",
      days: "Μέρες",
      hours: "Ώρες",
      minutes: "Λεπτά",
      seconds: "Δευτερόλεπτα",
      venueLabel: "Τοποθεσία",
      dateLabel: "Ημερομηνία",
      joinUs: "Ανυπομονούμε να γιορτάσουμε μαζί σας!",
    },
    schedule: {
      title: "Πρόγραμμα Γάμου",
      dayLabel: "Ημέρα",
      subtitle: "Τι να περιμένετε στην ξεχωριστή μας μέρα",
      days: [
        {
          title: "Ημέρα 1: Πέμπτη – Η Μυστική Συνάθροιση",
          panelTitle: "Πίνακας: Κοντινό σε ζεστό, φωτεινό οικογενειακό τραπέζι στο σπίτι της Μύρτου",
          panelText:
            "Οι θαρραλέοι ήρωες πυροδοτούν την θρυλική τους πορεία! Επανενώνονται με τους στενότερους συγγενείς στην κρυφή εστία του χώρου της Μύρτου για ένα γλέντι γεμάτο συγκινητικές ιστορίες και ανθεκτικούς δεσμούς. (Σσσ... αυτός ο ιερός πίνακας αποκαλύπτεται μόνο στους εκλεκτούς!)",
          locationName: "Σπίτι της Μύρτου",
          locationUrl: "https://maps.app.goo.gl/kn94JwutcQFEfdpg9",
        },
        {
          title: "Ημέρα 2: Παρασκευή – Η Μεγάλη Επανένωση",
          panelText:
            "Όλοι οι σύμμαχοι, συγκεντρωθείτε! Οι ήρωες ενώνονται με φίλους και οικογένεια στην ταβέρνα δίπλα στο κύμα, από το μεσημέρι μέχρι τα αστέρια να γεμίσουν τον ουρανό. Θα καταβροχθίσουμε επικά φαγητά, θα σκάσουμε στα γέλια και τα κύματα θα μας φορτίσουν ενέργεια για τη μεγάλη ένωση αύριο!",
          locationName: "Παραλία Alykes",
          locationUrl: "https://maps.app.goo.gl/k1vWCCXF9Bb79aAG7",
        },
        {
          title: "Ημέρα 3: Σάββατο – Η Απόλυτη Αποστολή Ξεκινά",
          panelText:
            "Η αυγή της αιωνιότητας ξημερώνει! Οι ήρωες ορμούν στο «Journey Beyond» στην κορυφή των επιβλητικών βουνών της Δίρφυς, στην Εύβοια, μέσα σε μυστικιστικό δάσος. Τελετή και γιορτή σας περιμένουν – ντυθείτε με την καλύτερη πανοπλία και ελάτε έτοιμοι για ένδοξες μάχες!",
          locationName: "Μαγικό Δάσος",
          locationUrl: "https://maps.app.goo.gl/ExUjAww6jhaXCp8w5",
        },
        {
          title: "Ημέρα 4: Κυριακή – Η Αυγή των Επιζώντων",
          panelText:
            "Οι νικητές αναδύονται από τη μάχη! Αυτοί που νίκησαν τις μαγικές δοκιμασίες συγκεντρώνονται για ένα χαλαρό πρωινό ρόφημα σε ένα αγαπημένο τοπικό στέκι. Αναλογιστείτε, επαναφορτιστείτε και κάντε πρόποση στους θρύλους που γεννήθηκαν!",
          locationName: "Κτήμα Ανθίδων",
          locationUrl: "https://maps.app.goo.gl/qGqXru5bec2JYz356",
        },
        {
          title: "Ημέρα 5: Δευτέρα – Η Άγρια Οδύσσεια",
          panelText:
            "Τολμηροί ήρωες διασχίζουν τα άγρια βουνά της Εύβοιας προς το Αιγαίο – μια επική αποστολή που καταλήγει σε μαγική παραλία. Ποιος ψήνεται?",
          locationName: "Παραλία Χιλιάδου",
          locationUrl: "https://maps.app.goo.gl/9vipXFbDTbtUQZGNA",
        },
      ],
    },
    rsvp: {
      title: "Επιβεβαίωση Παρουσίας",
      subtitle: "Παρακαλούμε ενημερώστε μας αν θα μπορέσετε να έρθετε",
      nameLabel: "Ονοματεπώνυμο",
      namePlaceholder: "Το πλήρες όνομά σας",
      emailLabel: "Διεύθυνση Email",
      emailPlaceholder: "το@email.σας",
      attendanceLabel: "Θα παραστείτε;",
      attendanceYes: "Αποδέχομαι με χαρά!",
      attendanceNo: "Δυστυχώς δεν θα μπορέσω",
      guestsLabel: "Αριθμός Ενηλίκων",
      kidsLabel: "Αριθμός Παιδιών",
      phoneLabel: "Τηλέφωνο",
      phonePlaceholder: "+30 69 1234 5678",
      messageLabel: "Οτιδήποτε θα θέλατε να προσθέσετε/πω;",
      messagePlaceholder: "Προσθέστε τυχόν σημειώσεις...",
      dietaryLabel: "Υπάρχουν διατροφικοί περιορισμοί;",
      dietaryPlaceholder: "Παρακαλώ αναφέρετε αλλεργίες ή διατροφικές ανάγκες",
      submit: "Αποστολή Επιβεβαίωσης",
      sending: "Αποστολή...",
      successTitle: "Ευχαριστούμε!",
      successMessage: "Η επιβεβαίωσή σας έχει ληφθεί. Ανυπομονούμε να γιορτάσουμε μαζί σας!",
      errorMessage: "Κάτι πήγε στραβά. Παρακαλώ δοκιμάστε ξανά.",
      deadline: "Παρακαλούμε επιβεβαιώστε μέχρι",
      sendAnother: "Αποστολή Άλλης Απάντησης",
      alreadySubmittedTitle: "Η Επιβεβαίωση Έχει Ήδη Σταλεί",
      alreadySubmittedMessage: "Έχετε ήδη υποβάλει την επιβεβαίωσή σας. Αν χρειαστεί να κάνετε αλλαγές, επικοινωνήστε μαζί μας απευθείας.",
      rentCarLabel: "Θα νοικιάσετε αυτοκίνητο;",
      rentCarYes: "Ναι",
      rentCarNo: "Όχι",
        eventDatesLabel: "Παρακαλούμε επιβεβαιώστε ποιες ημερομηνίες θα συμμετάσχετε στις",
        scheduleLinkText: "προγραμματισμένες εκδηλώσεις",
        eventDatesNote: "(Θα παρέχουμε διαμονή για τις 5 και 6 Ιουνίου)",
        eventOptions: {
          "2026-06-04": "<strong>4 Ιουνίου</strong>&nbsp;—&nbsp;Δείπνο με την πολύ κοντινή οικογένεια στο <a href=\"https://maps.app.goo.gl/kn94JwutcQFEfdpg9\">📍 Σπίτι της Μύρτου</a>",
          "2026-06-05": "<strong>5 Ιουνίου</strong>&nbsp;—&nbsp;Ολη την ημέρα στην παραλία Alykes στο <a href=\"https://maps.app.goo.gl/k1vWCCXF9Bb79aAG7\">📍 Παραλία Alykes</a>",
          "2026-06-06": "<strong>6 Ιουνίου</strong>&nbsp;—&nbsp;Τελετή γάμου στο <a href=\"https://maps.app.goo.gl/ExUjAww6jhaXCp8w5\">📍 Μαγικό Δάσος</a>",
          "2026-06-07": "<strong>7 Ιουνίου</strong>&nbsp;—&nbsp;Brunch στο <a href=\"https://maps.app.goo.gl/qGqXru5bec2JYz356\">📍 Κτήμα Ανθίδων</a>",
          "2026-06-08": "<strong>8 Ιουνίου</strong>&nbsp;—&nbsp;Ολη την ημέρα στην παραλία Χιλιάδου στο <a href=\"https://maps.app.goo.gl/9vipXFbDTbtUQZGNA\">📍 Παραλία Χιλιάδου</a>",
        },
    },
    touristicInfo: {
      title: "Ταξιδιωτικός Οδηγός",
      subtitle: "Όλα όσα χρειάζεται να γνωρίζετε για το ταξίδι σας",
      introText:
        "Εδώ έχουμε καταχωρίσει μερικά από τα αγαπημένα μας αξιοθέατα της περιοχής, για όσους θέλουν να εξερευνήσουν περαιτέρω την περιοχή. <em>Σημείωση: Αν θέλετε να επισκεφθείτε τη Σκιάθο ή τη Σκόπeλο, μη διστάσετε να επικοινωνήσετε για περισσότερες πληροφορίες και καθοδήγηση. Σας το προτείνουμε ανεπιφύλακτα για διακοπές.</em>",
      mapsLinkText: "Λίστα Google Maps",
    },
    weddingInfo: {
      title: "Λεπτομέρειες Γάμου",
      subtitle: "Σημαντικές πληροφορίες για τους καλεσμένους μας",
      dressCodeTitle: "Ενδυματολογικός Κώδικας",
      contactsTitle: "Επικοινωνία",
      giftsTitle: "Δώρα",
      accommodationTitle: "Διαμονή",
      contactNote: "Επικοινωνήστε μαζί μας για οτιδήποτε",
    },
    admin: {
      title: "Πίνακας Διαχείρισης",
      subtitle: "Διαχειριστείτε τους καλεσμένους και δείτε τις επιβεβαιώσεις",
      guestsTab: "Καλεσμένοι",
      rsvpsTab: "Επιβεβαιώσεις",
      totalGuests: "Σύνολο Ενηλίκων",
      totalRsvps: "Σύνολο RSVP",
      attending: "Παρόντες",
      declined: "Απόντες",
      addGuest: "Προσθήκη Καλεσμένου",
      editGuest: "Επεξεργασία Καλεσμένου",
      deleteGuest: "Διαγραφή",
      passwordLabel: "Κωδικός",
      passwordPlaceholder: "π.χ. giannis2026",
      passwordRequired: "Ο κωδικός είναι υποχρεωτικός",
      nameLabel: "Όνομα Καλεσμένου",
      namePlaceholder: "Όνομα καλεσμένου",
      groupLabel: "Ομάδα",
      groupFriends: "Φίλοι",
      groupFamily: "Οικογένεια",
      groupAdmin: "Διαχειριστής",
      save: "Αποθήκευση",
      noGuests: "Δεν υπάρχουν καλεσμένοι ακόμα",
      noRsvps: "Δεν υπάρχουν επιβεβαιώσεις ακόμα",
      rsvpEmail: "Email",
      rsvpPhone: "Τηλέφωνο",
      rsvpAttendance: "Παρουσία",
      rsvpYes: "Παρών",
      rsvpNo: "Απών",
      rsvpGuests: "Ενήλικες",
        rsvpKids: "Παιδιά",
      rsvpRentCar: "Ενοικίαση αυτοκινήτου",
      rsvpDietary: "Διατροφικοί περιορισμοί",
      rsvpMessage: "Μήνυμα",
      rsvpPassword: "Κωδικός που χρησιμοποιήθηκε",
    },
    footer: {
      madeWithLove: "Φτιαγμένο με αγάπη για",
    },
  },
} as const;

// Recursively widen string literal types to `string`
type Widen<T> = T extends string
  ? string
  : T extends object
  ? { [K in keyof T]: Widen<T[K]> }
  : T;

export type Dictionary = Widen<(typeof dictionaries)["en"]>;

export function getDictionary(locale: string): Dictionary {
  if (locale === "pt") return dictionaries.pt;
  if (locale === "el") return dictionaries.el;
  return dictionaries.en;
}
