...
function renderGiftsHtml(raw: string | undefined) {
  if (!raw) return "";
  // Split paragraphs by blank lines
  const paras = raw.split(/\n\s*\n/);
  const linked = paras
    .map((p) => {
      // escape HTML then restore markdown link syntax
      const escaped = escapeHtml(p).replace(/\n/g, "<br />");
      // convert [text](url) to anchors
      return escaped.replace(/\[(.*?)\]\((.*?)\)/g, (_m, text, href) => {
        const safeHref = href.startsWith("http") ? href : `https://${href}`;
        return `<a href="${safeHref}" target="_blank" rel="noopener noreferrer" class="text-vermillion/80 underline">${text}</a>`;
      });
    })
    .map((html) => `<p>${html}</p>`)
    .join("\n");
...
const MIRTO_SIGNAL_IMAGES = [
  "/images/mirtos_Signal_qrcode_crop.png",
];
                      ...
                      <div className="relative z-10 h-full w-full flex flex-col justify-end">
                        ...
                        <div className="manga-narration w-full">
                          {panel.item.isContacts ? (
                            ...
                            panel.item.title === dict.weddingInfo.giftsTitle ? (
                              <div
                                className="text-xs md:text-sm text-warm-gray leading-relaxed"
                                dangerouslySetInnerHTML={{ __html: renderGiftsHtml(panel.item.content) }}
                              />
                            ) : (
